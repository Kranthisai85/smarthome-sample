...
class VmFilterHelper
{
	...
	private function _oderByCategory($obj)
	{
		...
		switch ($_oderby){
			...
			case 2:
				usort ($obj, function ($a, $b) {
					return $a->_countProduct < $b->_countProduct;
				});
				break;
			case 3:
				usort ($obj, function ($a, $b) {
	...
	private function _getManuafactures_($manufactures)
	{
		$manufacturersModel = VmModel::getModel ('Manufacturer');
		$manufacturers = $manufacturersModel->getManufacturers (true,true,true);
		$list = array(); 	
		$_list = array();
		if (!empty($manufacturers) && !empty($manufactures)) {
			$productExtModel = new VirtuemartModelProductFilter();
			foreach ($manufacturers as $manu){
				if (in_array($manu->virtuemart_manufacturer_id, $manufactures))
				{
					$productExtModel->virtuemart_manufacturer_id = $manu->virtuemart_manufacturer_id;
					$productExtModel->sortSearchListQuery (true,0);
					$manu->_countProduct = $productExtModel->getTotal ();
	...
	public function _getCategoriesManuafactures($manufactures)
	{
		$list = array();
		...
		if ((int)$this->_params->get ('display_manuafactures',1)){
			$_manufacturers = $this->_getManuafactures_($manufactures);
			
			$list['manufactures'] = (!empty($_manufacturers))?$_manufacturers:null;
		}
	...
	public function _productFiltering($_arr_datas)
	{
		...
		if (!empty($_arr_datas)){
			...
			$_manufacturers = isset($_arr_datas['manufactures'])?$_arr_datas['manufactures']:0;
		
			if ($_manufacturers){
				$list['manufactures'] = $this->_getManuafactures ($_manufacturers);
			}
		...
		function sortSearchListQuery($onlyPublished = true,$virtuemart_category_id = false,$group = false,$nbrReturnProducts = false,$langFields = array())
		{
			...
			if ($useCore){
				...
				if ($this->_virtuemart_product_price){
					$currency = CurrencyDisplay::getInstance();
					$virtuemart_currency_id = $currency->getCurrencyForDisplay();
					$joinPrice = true;
					$_mf_price = $this->_virtuemart_product_price;
					$_mf_price_min = $_mf_price[0];
					$_mf_price_max = $_mf_price[1];
					if ($_mf_price_min != '' && $_mf_price_max != ''){
						$_mf_price_min = $currency->roundForDisplay($_mf_price_min, $virtuemart_currency_id);
						$_mf_price_max = $currency->roundForDisplay($_mf_price_max, $virtuemart_currency_id);
			...
			if ($this->_virtuemart_product_price){
				...
				foreach($products as $item){
					if (!empty($item->prices['salesPrice'])) {
						 $tmp_price = $currency->roundForDisplay($item->prices['salesPrice'], $virtuemart_currency_id);
					}
					if (!empty($item->prices['salesPriceWithDiscount'])) {
						 $tmp_price = $currency->roundForDisplay($item->prices['salesPriceWithDiscount'], $virtuemart_currency_id);
					} 
					$_mf_price = $this->_virtuemart_product_price;
					$_mf_price_min = $_mf_price[0];
					$_mf_price_max = $_mf_price[1];
					if ($_mf_price_min != '' && $_mf_price_max != ''){
						if ( $tmp_price >=  $_mf_price_min AND $tmp_price <= $_mf_price_max){
							$pro_ids[] = $item->virtuemart_product_id;
						}
						
					}
					if ($_mf_price_min == '' && $_mf_price_max != ''){
						if ( $tmp_price <= $_mf_price_max){
							$pro_ids[] = $item->virtuemart_product_id;
						}
					}

					if ($_mf_price_min != '' && $_mf_price_max == ''){
						if ( $tmp_price >= $_mf_price_min){
							$pro_ids[] = $item->virtuemart_product_id;
						}
					}
