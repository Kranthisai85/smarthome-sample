...
foreach ($viewData['products'] as $type => $products ) {
	...
	<ul class="row">
	    ...
	    // Show Products ?>
		<li class="item product col-md-4 col-sm-6 col-xs-12 col-lg-4 col-xl-<?php echo floor(12/$products_per_row); echo $show_vertical_separator ?>">
			<div class="item-inner clearfix">
				<div class="item-image">
					...
					<?php
					$percent = (int)(((((int)$product->prices['basePriceVariant'] - (int)$product->prices['salesPrice']))/$product->prices['basePriceVariant'])*100);

					if($percent) { ?>
						<span class="onsale"><?php echo '-'.$percent;?>%</span>
					<?php } ?>		
				</div>
				<div class="item-content">
					...
					<div class="item-description">
						...
						<?php if(!empty($rowsHeight[$row]['product_s_desc']) || !empty($rowsHeight[$row]['product_desc'])){
						?>
						<p class="product_s_desc">
							...
							if (!empty($product->product_desc)) {
								echo shopFunctionsF::limitStringByWord(strip_tags($product->product_desc), 200, ' ...') ?>
							<?php } ?>
						</p>
						<?php  } ?>
					</div>
