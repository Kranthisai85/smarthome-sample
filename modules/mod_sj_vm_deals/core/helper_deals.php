...
abstract class VMDealsHelper extends VMDealsBaseHelper {
	...
	public static function getList(&$params,$getC = 0)
	{
		...
		if(!empty($catids)){
			...
			foreach($items as $item){
				...
				$item->short_desc = $item->product_s_desc;
    ...
    private static function _getPublishCategories($catids)
    {
        ...
        !is_array($catids) && settype($catids, 'array');
        $db = JFactory::getDBO();
        $q = "SELECT virtuemart_category_id as id FROM #__virtuemart_categories as c WHERE c.published = 1";
        if (count($catids)>0) {
            $q .= " AND c.virtuemart_category_id IN ( " . implode(',', $catids) . " )";
        }
        $db->setQuery($q);
		...
		function sortSearchListQuery ($onlyPublished = TRUE, $virtuemart_category_id = FALSE, $group = FALSE, $nbrReturnProducts = FALSE, $langFields = Array()) {
			...
			if ($useCore) {
				...
				if ( $virtuemart_category_id !== false ){
					...
					$_virtuemart_category_id = array();
                    foreach ($virtuemart_category_id as $_vmcatid) {
                        if (isset($_vmcatid) && !empty($_vmcatid)) {
                            $_virtuemart_category_id[] = $_vmcatid;
                        }
                    }
                    
                    if (count($_virtuemart_category_id)>0) {
                        if (is_array($_virtuemart_category_id))
                            $where[] = ' `pc`.`virtuemart_category_id` IN ('.implode(',', $_virtuemart_category_id).')';
                        else
                            $where[] = ' `pc`.`virtuemart_category_id` IN ('.$_virtuemart_category_id.')';
                    }
