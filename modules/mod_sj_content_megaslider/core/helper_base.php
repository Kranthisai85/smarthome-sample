...
if (!class_exists('ContentMegaSliderBaseHelper')){
	/**
	 * ContentBaseHelper for com_content only.
	 */
	abstract class ContentMegaSliderBaseHelper{
		/**
		...
		*/
		...
		public static function getAImages($item, $params, $prefix='imgcfg'){
			...
			if ( !isset(self::$image_cache[$hash][$item->id]) ){
						...
						switch ($type){
							...
							case 'image_fulltext':
								if ( isset($images_data) && isset($images_data[$type]) && !empty($images_data[$type])){
									...
									$image['alt']  = ($image['alt'] != '')?$image['alt'] :$item->title;
									$image['title']  = ($image['title'] != '')?$image['title'] :$item->title;
				...
				$img_place_holder = $params->get($prefix.'_placeholder_path', 'modules/mod_sj_content_megaslider/assets/images/nophoto.jpg');
				if (count($images_path) == 0 && $params->get($prefix.'_placeholder', 1)==1){
					if(file_exists($img_place_holder) || @GetImageSize($img_place_holder) ){
						$images_path[] = array('src'=> $img_place_holder, 'title'=>$item->title, 'alt'=>$item->title, 'class'=>'placeholder');
					}else{
						$images_path[] = array('src'=> 'modules/mod_sj_content_megaslider/assets/images/nophoto.jpg', 'title'=>$item->title, 'alt'=>$item->title, 'class'=>'placeholder');
					}
				}
